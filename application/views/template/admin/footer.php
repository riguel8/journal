...
<script src="<?php echo base_url("./assets/js/apps/contact.js")?>"></script>
<script src="<?php echo base_url("./assets/libs/prism.js")?>"></script>
<script src="<?php echo base_url("./assets/js/ui-card-init.js?>")?>"></script>

<script src="<?php echo base_url("./assets/libs/js/select2.full.min.js")?>"></script>
...
<script>
document.getElementById('profilePicInputs').addEventListener('change', function(event) {
    const file = event.target.files[0]; // Get the selected file
    const reader = new FileReader(); // Create a FileReader instance

    // When the file is loaded
    reader.onload = function(e) {
        const img = document.getElementById('profilePics'); // Get the img element
        img.src = e.target.result; // Set the src attribute of the img element to the file data
